...
import { Plane, Users, Gauge, Briefcase } from 'lucide-react';
...
function extractAirportCode(cityString: string): string {
  ...
  return cityString.substring(0, 3).toUpperCase();
}
...
function extractMaxPassengers(passengersStr: string): number {
  // Patterns: "4-5 passengers", "Up to 9", "8 passengers", etc.
  const matches = passengersStr.match(/(\d+)/g);
  if (!matches) return 10; // Default fallback

  // If range (e.g., "4-5"), take the max number
...
export default function JetDetailClient({
  ...
  const dateParts = date.split('-'); // ['2025', '11', '27']
  const yearStr = dateParts[0];
  const monthNum = parseInt(dateParts[1]); // 1-12
  const dayOfMonth = parseInt(dateParts[2]);

  // Get month and day names without creating Date object
  const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const monthName = monthNames[monthNum - 1];

  // Only use Date object for getting day of week
  const tempDate = new Date(`${date}T12:00:00`); // ISO format at noon
  ...
  const handlePassengerChange = (value: string) => {
    ...
    // Don't allow more passengers than the aircraft can hold
