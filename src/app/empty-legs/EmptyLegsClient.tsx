...
interface EmptyLegsClientProps {
  initialEmptyLegs: EmptyLeg[]
}

const ITEMS_PER_PAGE = 12

export default function EmptyLegsClient({ initialEmptyLegs }: EmptyLegsClientProps) {
  ...
  const priceRange = useMemo(() => {
    if (initialEmptyLegs.length === 0) {
      return { min: 0, max: 50000 }
    }

    const prices = initialEmptyLegs.map(leg => leg.discountedPrice)
    return {
      min: Math.min(...prices),
      max: Math.max(...prices)
    }
  ...
  const filteredEmptyLegs = useMemo(() => {
    return filterEmptyLegs(initialEmptyLegs, filters)
  }, [initialEmptyLegs, filters])

  // Calculate pagination
  const totalPages = Math.ceil(filteredEmptyLegs.length / ITEMS_PER_PAGE)
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
  const endIndex = startIndex + ITEMS_PER_PAGE
  const currentEmptyLegs = filteredEmptyLegs.slice(startIndex, endIndex)
          ...
          <div className="flex items-center justify-center min-h-[600px]">
            <div className="max-w-2xl text-center space-y-6 px-6">
              ...
              <div className="space-y-2">
                ...
                >
                  We couldn't find an Empty Leg matching your criteria at the moment.
                </p>
              </div>
              ...
              <div className="pt-4">
                <p
                  ...
                  style={{ fontFamily: 'Montserrat, sans-serif' }}
                >
                  You can also try adjusting your filters to see more available Empty Legs
                </p>
              </div>
            </div>
          </div>
